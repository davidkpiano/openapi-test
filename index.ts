...
const host = isLocal
    ? "localhost:5000"
    : yargs.argv.host as string || process.env.OPENAPI_HOST as string;
            ...
            (err, actualResponse) => {
                ...
                if (test.response.status) {
                    ...
                    if (
                        ...
                        const schemaValid = ajv.validate(
                            spec,
                            actualResponse.body
                        );
...
const swaggerSpec = JSON.parse(
    fs.readFileSync(path.join(__dirname, "../src/swagger.json"), {
        encoding: "utf-8"
    })
);
